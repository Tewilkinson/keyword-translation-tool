...
if not OPENAI_API_KEY:
    st.warning("OPENAI_API_KEY is not set (secrets or env). The translate button will fail.")

supabase: Client = create_client(SUPABASE_URL, SUPABASE_KEY)
openai_client = OpenAI(api_key=OPENAI_API_KEY) if OPENAI_API_KEY else None

st.title("🌍 Keyword Translation Tool")
st.caption("Input: keyword, category, subcategory, product_category. Only keyword is translated into two variants; other fields are preserved.")
...
for job in jobs:
    ...
    # Per-job diagnostics: show counts
    ...
    st.caption(f"Items: {done}/{total} translated")
    ...
    for job in jobs:
        ...
        try:
            with open(tmp, "rb") as f:
                supabase.storage.from_("translated_files").upload(f"results/{fname}", f, {"cacheControl":"3600", "upsert": True})
